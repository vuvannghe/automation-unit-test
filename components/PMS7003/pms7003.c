...
esp_err_t pms7003_initUart(uart_config_t *uart_config, uart_port_t uart_port, gpio_num_t pms_tx_pin, gpio_num_t pms_rx_pin)
{
    /**
     ...
     */
    esp_err_t error_1 = uart_driver_install(uart_port, (RX_BUFFER_SIZE * 2), 0, 0, NULL, 0);
    ESP_ERROR_CHECK_WITHOUT_ABORT(error_1);
    esp_err_t error_2 = uart_param_config(uart_port, uart_config);
    ESP_ERROR_CHECK_WITHOUT_ABORT(error_2);
    esp_err_t error_3 = uart_set_pin(uart_port, pms_tx_pin, pms_rx_pin, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE);
    ...
    if (error_1 == ESP_OK &&
        ...
        error_3 == ESP_OK)
    {
        ESP_LOGI(__func__, "PMS7003 UART port initialize successfully.");
        return ESP_OK;
    }
    else
    {
    ...
    for (size_t i = 0; i < 104; i++)
    {
            ...
            rawData[i + 1] == START_CHARACTER_2) // Check second start byte in raw data array.
        {
            ...
            *pm10 = ((rawData[startByte + 4] << 8) + rawData[startByte + 5]);
            ...
            ESP_LOGI(__func__, "PMS7003 sensor read data successfully.");
